function [net, info] = cnn_finetune_train(net, imdb, getBatch, varargin)
...
% the terms of the BSD license (see the COPYING file).
    ...
    if isstr(opts.errorFunction)
      switch opts.errorFunction
        ...
        case 'diagnosis'
          opts.errorFunction = @auc_roc ;
          if isempty(opts.errorLabels), opts.errorLabels = {'AUC-ROC'} ; end
        otherwise
    ...
    % setup model and figures paths
    modelPath = @(ep) fullfile(opts.expDir, sprintf('net-epoch-%d.mat', ep));
    modelFigPath = fullfile(opts.expDir, 'net-train.pdf') ;
    modelFigPath_fig = fullfile(opts.expDir, 'net-train.fig') ;

    % resume from last checkpoint when possible
    ...
    for epoch = start+1 : opts.numEpochs
      ...
      if rem(epoch, 10) == 0
          learningRate = learningRate / 10^(epoch / 10);
      end
      ...
      % process epoch for training/validation data
      if numGpus <= 1
      ...
      if ~evaluateMode
        ...
        tic ;
        if epoch ~= 0
            if exist(fullfile(modelPath(epoch - 1)), 'file')
      ...
      if opts.plotStatistics
        ...
        xlabel('Training epoch') ; ylabel('Cost function') ;
        grid on ;
        h=legend(sets) ;
        set(h,'color','none');
        title('Objective') ;
        if hasError
          ...
          if ~evaluateMode
            ...
            hold on ;
            leg = horzcat(leg, strcat('Training ', opts.errorLabels)) ;
          end
          plot(1:epoch, info.val.error', '.--') ;
          leg = horzcat(leg, strcat('Validation ', opts.errorLabels)) ;
          set(legend(leg{:}),'color','none') ;
          grid on ;
          xlabel('Training epoch') ; ylabel(opts.errorLabels) ;
          title('Performance') ;
        end
        drawnow ;
        print(1, modelFigPath, '-dpdf') ;
        savefig(gcf, modelFigPath_fig);
      end
    end
end
...
function err = auc_roc(opts, labels, res)
    predictions = squeeze( gather(res(end-1).x) )';
    predictions = predictions(:,2);
    [~,~,info] = vl_roc(2 * (labels-1) - 1, predictions);
    err = info.auc * length(predictions) ;
end
